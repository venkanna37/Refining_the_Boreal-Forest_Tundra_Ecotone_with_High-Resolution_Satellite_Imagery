...
def apply_model_on_geotiff(
    ...
    for i, path in enumerate(checkpoint_path):
        model = unet.UNet(input_channels=1)
    ...
    with torch.no_grad():
        ...
        # print(mean, std, image.min(), image.max())
        pred = ensemble_union_predict(models, patch)[0, 0]

    output_mask = pred.cpu().numpy()
    # output_mask = np.digitize(output_mask, np.arange(0, 1.1, 0.1))
...
def main():
    ...
    # keywords = ['UNetW300F1', 'UNetW300F2', 'UNetW300F3', 'UNetW300F4']
    # keywords = ['GF1NAug', 'GF2NAug', 'GF3NAug', 'GF4NAug', 'GF5NAug']
    # keywords = ['GF1YAug', 'GF2YAug', 'GF3YAug', 'GF4YAug', 'GF5YAug']
    # keywords = ['GF1YAugW', 'GF2YAugW', 'GF3YAugW', 'GF4YAugW', 'GF5YAugW']
    keywords = ['GF1NAugS', 'GF2NAugS', 'GF3NAugS', 'GF4NAugS', 'GF5NAugS']
